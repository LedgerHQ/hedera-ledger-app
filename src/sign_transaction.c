    ...
    unsigned int button_mask_counter)
{
    ...
    case BUTTON_EVT_RELEASED | BUTTON_RIGHT:
        if (ctx.type == Verify || ctx.type == Associate)
        {
            ctx.step = Senders;
            ctx.display_index = 1;
            reformat_senders();
...
void handle_intermediate_left_press()
{
    ...
    case Amount:
    {
        if (first_screen())
        {
            ...
            else if (ctx.type == Transfer || ctx.type == TokenTransfer)
...
void reformat_tokens_accounts(char *title_part, uint8_t transfer_index)
{
    hedera_snprintf(
        ...
        ACCOUNT_ID_SIZE,
        "%llu.%llu.%llu",
        ctx.transaction.data.cryptoTransfer.tokenTransfers[0].accountAmounts[transfer_index].accountID.shardNum,
        ctx.transaction.data.cryptoTransfer.tokenTransfers[0].accountAmounts[transfer_index].accountID.realmNum,
        ctx.transaction.data.cryptoTransfer.tokenTransfers[0].accountAmounts[transfer_index].accountID.accountNum);
...
{
    if (ctx.type == Associate)
    {
        reformat_token();
    }
    else if (ctx.type == TokenTransfer)
    {
        reformat_tokens_accounts("Sender", ctx.transfer_from_index);
    }
    else if (ctx.type == Verify)
    {
...
void handle_transaction_body()
{
    ...
    case HederaTransactionBody_cryptoTransfer_tag:
    {
            ...
            ctx.transaction.data.cryptoTransfer.transfers.accountAmounts_count == 2)
        {
            ...
            ctx.transaction.data.cryptoTransfer.tokenTransfers_count == 1)
        {
...
void validate_transfer()
{
    ...
    {
        if (ctx.transaction.data.cryptoTransfer.tokenTransfers[0].accountAmounts_count != 2)
        {
    ...
    /* out */ volatile unsigned int *tx)
{
    ...
    // Decimals
    // Hopefully, the Hedera Protos for Tokens will contain decimal information at
    // some point, but for now, if performing a token transfer, the decimal value
    // for that token should be passed. sorry
    ctx.decimals = p1;
